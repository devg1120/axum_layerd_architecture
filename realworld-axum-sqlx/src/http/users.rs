...
async fn create_user(
    ...
    let user_id = sqlx::query_scalar!(
        // language=PostgreSQL
        r#"insert into "user2" (username, email, password_hash) values ($1, $2, $3) returning user_id"#,
        req.user.username,
        req.user.email,
        password_hash
    )
...
async fn get_current_user(
    ...
    let user = sqlx::query!(
        r#"select email, username, bio, image from "user2" where user_id = $1"#,
        auth_user.user_id
    )
    .fetch_one(&ctx.db)
    .await?;
...
async fn update_user(
    ...
    let user = sqlx::query!(
        ...
        r#"
            update "user2"
            set email = coalesce($1, "user2".email),
                username = coalesce($2, "user2".username),
                password_hash = coalesce($3, "user2".password_hash),
                bio = coalesce($4, "user2".bio),
                image = coalesce($5, "user2".image)
            where user_id = $6
