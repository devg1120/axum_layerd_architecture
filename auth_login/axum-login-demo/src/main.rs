...
async fn login_handler(
    ...
    println!("password_hash:{}", user.password_hash);
    println!("login password:{}", login.password);

    //let is_valid = bcrypt::verify(user.password, &hash_password).unwrap();
    let is_valid = bcrypt::verify(login.password, &user.password_hash).unwrap();
